...
model.add(tf.layers.dense({units: 128, activation: "linear", dropout: dropout }));
...
model.compile({optimizer: tf.train.rmsprop(0.0002), loss: tf.losses.meanSquaredError , metrics: ['accuracy'] });
...
if (canTrain ) {
	...
	data.copy(buf, RATE/4*(SZ-0), 0, RATE/4)
